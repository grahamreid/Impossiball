...
public class ControlBall : MonoBehaviour {
	...
	void Update () {
		...
		if (floatRotationX < -floatRotationSpeed / 4 || floatRotationX > floatRotationSpeed / 4) {
			...
			//if(Vector3.Project(this.rigidbody.angularVelocity,objOrientation.transform.TransformDirection(Vector3.forward)).magnitude > floatMaxAngularVelocity)
				//this.rigidbody.AddTorque(v3ForwardCounterRotation);
			//print ("original"+this.rigidbody.angularVelocity.magnitude);
			//print ("projection"+Vector3.Project(this.rigidbody.angularVelocity,objOrientation.transform.TransformDirection(Vector3.forward)).magnitude);
				}
		if(floatRotationZ < -floatRotationSpeed/4 || floatRotationZ > floatRotationSpeed/4){
			this.rigidbody.AddTorque (v3SideRotation);
			//if(Vector3.Project(this.rigidbody.angularVelocity,objOrientation.transform.TransformDirection(Vector3.right)).magnitude > floatMaxAngularVelocity)
				//this.rigidbody.AddTorque(v3SideCounterRotation);
			//print ("original"+this.rigidbody.angularVelocity.magnitude);
			//print ("projection"+Vector3.Project(this.rigidbody.angularVelocity,objOrientation.transform.TransformDirection(Vector3.right)).magnitude);
